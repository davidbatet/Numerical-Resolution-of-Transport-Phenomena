...
def assemble_matrix_and_source_smith_hutton(
    ...
    for j in range(Ny):
        for i in range(Nx):
            ...
            # East face
            if i < Nx - 1:
                De = Gamma_P * Ae / dx_E[i]

                x_face = x_e[i + 1]
                y_face = y_c[j]

                Fe = rho * u_field(x_face, y_face) * Ae
                ...
                delta_e = dx_cell[-1] / 2.0
                De_boundary = Gamma_P * Ae / delta_e
                ...
                if scheme == "upwind":
                    aE_boundary = De_boundary + max(-Fe, 0.0)
                else:
                    Pe = Fe / De_boundary if De_boundary != 0.0 else 0.0
                    aE_boundary = (
                        De_boundary * power_law_A(Pe)
                        + max(-Fe, 0.0)
                    )
            ...
            # West face
            if i > 0:
                Dw = Gamma_P * Aw / dx_W[i]

                x_face = x_e[i]
                y_face = y_c[j]

                Fw = rho * u_field(x_face, y_face) * Aw
            ...
            # North face
            if j < Ny - 1:
                Dn = Gamma_P * An / dy_N[j]

                x_face = x_c[i]
                y_face = y_e[j + 1]

                Fn = rho * v_field(x_face, y_face) * An
                ...
                delta_n = dy_cell[-1] / 2.0
                Dn_boundary = Gamma_P * An / delta_n
                ...
                if scheme == "upwind":
                    aN_boundary = Dn_boundary + max(-Fn, 0.0)
                else:
                    Pe = Fn / Dn_boundary if Dn_boundary != 0.0 else 0.0
                    aN_boundary = (
                        Dn_boundary * power_law_A(Pe)
                        + max(-Fn, 0.0)
                    )
            ...
            # South face
            if j > 0:
                Ds = Gamma_P * As / dy_S[j]

                x_face = x_c[i]
                y_face = y_e[j]

                Fs = rho * v_field(x_face, y_face) * As
                ...
                if x_here < 0.0:
                    phi_boundary = 1.0 + np.tanh(
                        alpha * (2.0 * x_here + 1.0)
                    )

                    delta_s = dy_cell[0] / 2.0
                    Ds_boundary = Gamma_P * As / delta_s

                    if scheme == "upwind":
                        aS_boundary = Ds_boundary + max(Fs, 0.0)
                    else:
                        Pe = Fs / Ds_boundary if Ds_boundary != 0.0 else 0.0
                        aS_boundary = (
                            Ds_boundary * power_law_A(Pe)
                            + max(Fs, 0.0)
                        )
