"""
2D differentially heated cavity on a uniform MAC grid.
...
# Plot selector
PLOT = "pressure"  # "T", "speed", "pressure", "stream", "T+stream", "vorticity", "Nu_hot"
CMAP = "turbo"
LEVELS = 60
...
dt_conv = Cconv * min(
    dx / max(umax0, 1.0e-14),
    dy / max(vmax0, 1.0e-14),
)

dt_diff_u = Cdiff * min(dx * dx, dy * dy) / max(Pr, 1.0e-14)
dt_diff_T = Cdiff * min(dx * dx, dy * dy)
...
Nu_hot_local, Nu_hot_avg, Nu_cold_local, Nu_cold_avg = compute_nusselt_hot_cold(
    T=T,
    dx=dx,
    Thot=Thot,
    Tcold=Tcold,
...
if EXPORT_VTK:
    ...
    vtk_path = os.path.join(vtk_dir, "differentially_heated_cavity_uniform.vtk")
...
if PLOT != "Nu_hot":
    plt.xlabel("x")
    plt.ylabel("y")
    plt.title(f"DHC uniform mesh, Ra={Ra:g}, Pr={Pr:g}, plot={PLOT}")
