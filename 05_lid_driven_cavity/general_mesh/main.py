"""
2D lid-driven cavity flow on a general non-uniform MAC grid.
...
# Plot selector
PLOT = "speed+stream"   # "speed", "pressure", "u", "v", "stream", "speed+stream", "quiver", "vorticity"
...
dx_min = float(np.min(grid["dx_cell"]))
dy_min = float(np.min(grid["dy_cell"]))

umax = U_lid
vmax = U_lid

dt_conv = Cconv * min(
    dx_min / max(umax, 1.0e-14),
    dy_min / max(vmax, 1.0e-14),
)

dt_visc = Cvisc * min(dx_min * dx_min, dy_min * dy_min) / nu
...
if EXPORT_VTK:
    ...
    vtk_path = os.path.join(vtk_dir, "lid_driven_cavity_general.vtk")
...
mesh_tag = f"tanh beta=({beta_x}, {beta_y})"
plt.title(f"Lid-driven cavity general mesh, Re = {Re:g}, {mesh_tag}, plot = {PLOT}")
